...
def collate(
    ...
    def _collate_constraint_masks(frames: List[torch.Tensor]):
        """
        Convert a list of 2D frames into a padded 3D tensor
        Args:
            frames (list): list of 2D frames of size L[i]*f_dim. Where L[i] is
    ...
    if samples[0].get("constraint_masks", None) is not None:
        constraint_masks = _collate_constraint_masks(
            [s["constraint_masks"] for s in samples]
        )
...
class UnifyDataset(OFADataset):
    ...
    def process_pure_text(self, index):
        ...
        constraint_masks = torch.stack(
            [
                torch.arange(len(self.tgt_dict))
                < len(self.tgt_dict)
                - self.audio_code_dict_size
                - self.code_dict_size
                - self.num_bins
                for _ in range(len(target_item))
            ]
        )
    ...
    def process_speech_text_pair(self, index, dataset=None):
        ...
        fbank = self.prepare_fbank(
            torch.tensor([wav], dtype=torch.float32), sr, speed, speech_id
        )
        ...
        with data_utils.numpy_seed(self.seed, self.epoch):
            if self.train_stage == 1:
                ...
                if self.dataset is not None:
                    extra_samples += (
                        self.process_pure_text(index)
                        if not self.remove_pure_text
                        else []
                    )
                ...
                if self.split == "train":
                    if self.dataset is not None:
                        audio_examples += (
                            self.process_pure_audio(index)
                            if not self.remove_pure_audio
                            else []
                        )
                    if (
                        self.speech_text_dataset is not None
                        and self.dataset.data_cnt % 4 == 0
                    ):
                        pair_examples += self.process_speech_text_pair(index)
                    if (
                        self.pure_text_dataset is not None
                        and self.dataset.data_cnt % 2 == 0
                    ):
                        extra_samples += (
                            self.process_pure_text(index)
                            if not self.remove_pure_text
                    ...
                    if self.dataset is not None:
                        pair_examples += self.process_speech_text_pair(
                            index, self.dataset
                        )
    ...
    def collater(self, samples, pad_to_length=None):
        ...
        samples_v1 = (
            []
        )  # containing phone-text pairs at stage-1, containing speech-text pairs at stage-2
        ...
        if self.split == "train" and self.train_stage != 1:
            if samples_v2 == []:
                if self.pure_text_dataset is not None:
                    samples_v2 += (
                        self.process_pure_text(0) if not self.remove_pure_text else []
                    )
            res_v2 = collate(samples_v2, pad_idx=self.src_dict.pad(), eos_idx=self.eos)
            if samples_v3 == []:
                if self.pure_audio_dataset is not None:
                    samples_v3 += (
                        self.process_pure_audio(0) if not self.remove_pure_audio else []
                    )
                else:
